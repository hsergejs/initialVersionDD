...
<? if (stristr($_SERVER['PHP_SELF'],"DermaDental.php")) { 
      
    die("We apologise, but this company has been removed from our catalogues."); 
}
?>
<? include ("block/bd.php");
$result = mysql_query ("SELECT * FROM ltd_page WHERE sh_name='DermaDental'",$db);
if (!$result)
{
...
//RATING SCRIPT
$queryr = mysql_query ("SELECT * FROM rating WHERE sh_name='DermaDental'");
...
//END RATING SCRIPT
//COMMENT SCRIPT
$queryget = mysql_query ("SELECT * FROM comments WHERE sh_name='DermaDental' ORDER BY id ASC") or die("Error 3");
